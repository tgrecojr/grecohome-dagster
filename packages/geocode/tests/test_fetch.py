...
class TestReverseGeocode:
    ...
    def test_ok_returns_raw_bytes(self):
        ...
        out = fetch.reverse_geocode(
            39.8, -75.1, base_url=BASE, language="en", radius_km=0.5, limit=10
        )
        assert out == FC
        ...
        assert req.url.params["radius"] == "0.5"
        assert req.url.params["limit"] == "10"
        assert req.url.params["distance_sort"] == "true"

    @respx.mock
    def test_omits_radius_and_limit_when_none(self):
        route = respx.get(REVERSE).mock(return_value=_ok())
        fetch.reverse_geocode(39.8, -75.1, base_url=BASE, radius_km=None, limit=None)
