...
**continuous date spine** (so gaps are explicit for rolling/streak analysis), left-joining the
silver tables:
...
| `silver_strain` | `cycle_id = recovery.cycle_id` | the day's exertion next to its recovery (kJ → kcal) |
| `silver_daily` | `activity_date = day` (1:1) | steps, active cal, distance, floors, intensity, stress, body-battery |
...
| `silver_body` | **ASOF** `day >= measured_date` | latest weigh-in **carried forward** (kg → lb) |

Notes:
- **Recovery dedup.** Recovery's `recovery_date` is the UTC date of `created_at` and is occasionally
  2-per-date, so the mart keeps the latest `created_at` per day.
- **Strain** attaches to that kept recovery's **`cycle_id`** (same physiological cycle), so each day's
  strain sits beside its recovery; `kilojoules` is converted to `strain_kilocalories`.
- **Weight** is **as-of joined** — weigh-ins are sparse, so each day carries the most recent weigh-in
  on or before it (`weight_lb` = `weight_kg` × 2.20462), with BMI / body-fat.
- **Glucose time-in-range** uses **70–140 mg/dL**: `glucose_tir_pct`, `glucose_pct_below` (<70),
  `glucose_pct_above` (>140).
- **Workouts** can be many per day → aggregated; `workout_count` is 0 (not null) on days with none.
- **Provenance:** `has_sleep` / `has_recovery` / `has_strain` / `has_daily` / `has_workout` /
  `has_glucose` / `has_weight` make every null explainable.
...
`resting_heart_rate`, `hrv_rmssd_milli`, `spo2_percentage`; strain: `day_strain`,
`strain_kilocalories`, `strain_avg_hr`, `strain_max_hr`; daily activity: `steps`,
`active_calories`, `distance_km`, `floors`, `intensity_minutes`, `avg_stress`,
`body_battery_high`/`_low`; workouts: `workout_count`, `workout_total_min`,
`workout_distance_km`, `workout_calories`; glucose: `glucose_readings`, `glucose_mean`,
`glucose_min`, `glucose_max`, `glucose_std`, `glucose_tir_pct`, `glucose_pct_below`,
`glucose_pct_above`; body: `weight_kg`, `weight_lb`, `body_bmi`, `body_fat_pct`; `has_*` flags.

### Asset checks
| Check | Severity | What |
|---|---|---|
| `wellness_day_unique_nonnull` | ERROR | one row per `day`, never null |
| `wellness_value_ranges` | ERROR | TIR/glucose/recovery + strain 0–21, steps, weight, body-fat, stress in bounds |
| `wellness_coverage` | WARN | per-source day coverage (incl. strain / daily / weight); fails only if empty |
