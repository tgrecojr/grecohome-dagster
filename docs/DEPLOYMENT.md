...
`dagster==1.13.17` and all `dagster-*==0.29.17` must match the host daemon/webserver so the
...
In Dagster 1.13.17 a **per-pool limit is set in the instance DB via the CLI**, not in
...
- **`BRONZE_ROOT` — read-only** (`/opt/datalake/bronze` → `/data/bronze:ro`). Silver reads the
  source segments it projects (`garmin/`, `whoop/`, `lingo/`, `uscrn/`).
