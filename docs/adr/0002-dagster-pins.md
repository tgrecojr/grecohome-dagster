...
## Decision
- **Pin `dagster==1.13.17` and all `dagster-*==0.29.17`** to match the host exactly, so the gRPC
...
- **Host owns concurrency.** The shared `whoop_api` pool's limit is set on the host instance
  ...
  the image. In 1.13.17 a per-pool limit can't be named in `dagster.yaml` (its `concurrency.pools`
